...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		poseIncrement.apply(source,result);
		
		T target[3];
		
		fillVector<T>(m_targetPoint,target);
		//residuals[0] = (T)m_weight*(target[0] - result[0]);
		//residuals[1] = (T)m_weight*(target[1] - result[1]);
		//residuals[2] = (T)m_weight*(target[2] - result[2]);
		residuals[0] = (target[0] - result[0]);
		residuals[1] = (target[1] - result[1]);
		residuals[2] = (target[2] - result[2]);
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		//residuals[1] = (T)m_weight*(target[1] - result[1]);
		//residuals[2] = (T)m_weight*(target[2] - result[2]);
		residuals[0] = (target[0]-result[0])*normal[0]
					+ (target[1]-result[1])*normal[1]
					+ (target[2]-result[2])*normal[2];
	...
	void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				problem.AddResidualBlock(PointToPointConstraint::create(sourcePoint, targetPoint, 1.0), NULL, poseIncrement.getData());
				...
				if (m_bUsePointToPlaneConstraints) {
					...
					problem.AddResidualBlock(PointToPlaneConstraint::create(sourcePoint, targetPoint,targetNormal, 1.0), NULL, poseIncrement.getData());
